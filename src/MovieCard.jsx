...
const MovieCard = ({ Title, PosterImg, VoteAverage, Overview, Release, Genre, trailerKey }) => {
  ...
  return (
    <section className="MovieCard">
      <img src={imgUrl} alt={Title} className="PosterImg" onClick={handleOpen} style={{cursor: "pointer"}} />
      <section className="movie-info">
        <h2 className="MovieTitle">{Title}</h2>
        <h3 className="VoteAverage">Vote Average: {VoteAverage}</h3>
        <div className="button-container">
          <button
            className={`movie-button favorite-button ${isFavorite ? 'active' : ''}`} // differentiate between active button and non active button
            onClick={handleFavoriteToggle}
          >
            {isFavorite ? 'Favorite' : 'Favorite'}
          </button>
          <button
            className={`movie-button watched-button ${isWatched ? 'active' : ''}`}
